...
- `"passkey"` - [Passkey](./passkeys.md) auth (in preview)

## `allowedLoginMethods`

The list of login methods that are permitted for auth. This defaults to `['password','header','openid','passkey']` (environment variable: `ACTUAL_ALLOWED_LOGIN_METHODS`, comma separated string).
